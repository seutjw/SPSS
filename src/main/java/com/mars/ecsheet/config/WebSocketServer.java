 ...
 * @description
 */
@ServerEndpoint("/ws/{userId}/{username}/{gridKey}")
@Component
public class WebSocketServer {
     ...
     */
    @OnOpen
    public void onOpen(Session session, @PathParam("userId") String userId, @PathParam("username") String username, @PathParam("gridKey") String gridKey) {
                    ...
                    sessionMap.forEach((key, value) -> {
                        ...
                        if (!key.equals(this.userId)) {
                            try {
                                //如果是mv,代表发送者的表格位置信息
                                if ("mv".equals(jsonObject.getStr("t"))) {
                                    value.sendMessage(JSONUtil.toJsonStr(ResponseDTO.mv(userId, username, unMessage)));

                                    //如果是切换sheet，则不发送信息
                                } else if(!"shs".equals(jsonObject.getStr("t"))) {
                                    value.sendMessage(JSONUtil.toJsonStr(ResponseDTO.update(userId, username, unMessage)));
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
