...
public class ExcelUtils {
	public static CellStyle createCellStyle(XSSFSheet sheet, XSSFWorkbook wb, JSONObject jsonObjectValue) {
		...
		fontMap.put(12, "华文隶书");
		// 合并单元格
		if (jsonObjectValue.get("mc") != null && ((JSONObject) jsonObjectValue.get("mc")).get("rs") != null
				&& ((JSONObject) jsonObjectValue.get("mc")).get("cs") != null) {
		...
		if (jsonObjectValue.get("un") != null) {
			font.setUnderline(
					"1".equals(jsonObjectValue.get("un").toString()) ? FontUnderline.SINGLE : FontUnderline.NONE);
		}
		// 字体大小
	...
	public static void exportLuckySheetXlsx(String excelData, HttpServletRequest request,
		...
		for (int i = 0; i < jsonObjects.size(); i++) {
			...
			// 默认高
			short defaultRowHeight = jsonObject.getShort("defaultRowHeight") == null ? 20
					: jsonObject.getShort("defaultRowHeight");
			// 默认宽
			short defaultColWidth = jsonObject.getShort("defaultColWidth") == null ? 74
					: jsonObject.getShort("defaultColWidth");
		...
		try {
			...
			String name = exportTime + "_excel_file.xlsx";
			if (request != null && request.getHeader("USER-AGENT") != null
					&& StringUtils.contains(request.getHeader("USER-AGENT"), "Firefox")) {
				disposition += new String((name).getBytes(), "ISO8859-1");
			} else {
				disposition += URLEncoder.encode(name, "UTF-8");
			}
			response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8");
			response.setHeader("Content-Disposition", disposition);
			// 修改模板内容导出新模板
			File file = new File("G:\\luckysheet-import-team-edit-export-master\\src\\main\\java\\com\\mars\\ecsheet" +
					"\\exceldata\\"+id+".xlsx");
			//System.out.println("G:\\luckysheet-import-team-edit-export-master\\src\\main\\java\\com\\mars\\ecsheet" +
			//		"\\exceldata\\"+request.getParameter("wbid")+".xlsx");
			if(!file.exists())
			{
	...
	public static Map<String, Integer> getColRowMap(JSONObject imageDefault, short defaultRowHeight,
			short defaultColWidth, JSONObject columnlenObject, JSONObject rowlenObject) {
...
//        int dy2 = top+height;
		...
		map.put("dx1",
				dx1.multiply(new BigDecimal(Units.EMU_PER_PIXEL)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue());
		map.put("dy1",
				dy1.multiply(new BigDecimal(Units.EMU_PER_PIXEL)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue());
		map.put("dx2",
				dx2.multiply(new BigDecimal(Units.EMU_PER_PIXEL)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue());
		map.put("dy2",
				dy2.multiply(new BigDecimal(Units.EMU_PER_PIXEL)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue());
		map.put("col1", col1);
		map.put("row1", row1);
		map.put("col2", col2);
		map.put("row2", row2);
			...
			for (Map.Entry<String, Object> rowEntry : rowMap.entrySet()) {
				...
				if (columnlenObject != null) {
					...
					for (Map.Entry<String, Object> cloEntry : cloMap.entrySet()) {
						...
						sheet.setColumnWidth(Integer.parseInt(cloEntry.getKey()),
								wid.multiply(excleWid).setScale(0, BigDecimal.ROUND_HALF_UP).intValue());// 列宽px值
	...
	private static void setCellValue(XSSFWorkbook wb, XSSFSheet sheet, JSONArray jsonObjectList,
			JSONObject columnlenObject, JSONObject rowlenObject, short defaultRowHeight, short defaultColWidth) {
		for (int index = 0; index < jsonObjectList.size(); index++) {
			...
			if (jsonObjectValue.get("ps") != null) {
				...
				Map<String, Integer> colrowMapPS = getColRowMap(ps, defaultRowHeight, defaultColWidth, columnlenObject,
						rowlenObject);
				XSSFClientAnchor anchor = new XSSFClientAnchor(colrowMapPS.get("dx1"), colrowMapPS.get("dy1"),
						colrowMapPS.get("dx2"), colrowMapPS.get("dy2"), colrowMapPS.get("col1"),
						colrowMapPS.get("row1"), colrowMapPS.get("col2"), colrowMapPS.get("row2"));
	...
	private static void settDataValidation(JSONObject dataVerification, XSSFSheet sheet) {
		...
		if (dataVerification != null) {
			...
			for (Map.Entry<String, Object> dataEntry : dataVe.entrySet()) {
				...
				CellRangeAddressList dstAddrList = new CellRangeAddressList(Integer.parseInt(colRow[0]),
						Integer.parseInt(colRow[0]), Integer.parseInt(colRow[1]), Integer.parseInt(colRow[1]));// 规则一单元格范围
				...
				if ("dropdown".equals(dataVeValue.getString("type"))) {
					if (dataVeValue.getString("value1").contains(",")) {
						...
						dstDataValidation = helper.createValidation(helper.createExplicitListConstraint(textlist),
								dstAddrList);
					} else {
						dstDataValidation = helper.createValidation(
								helper.createFormulaListConstraint(dataVeValue.getString("value1")), dstAddrList);
					}
				}
				...
				if ("number".equals(dataVeValue.getString("type"))) {
					...
					if (booleanValue1 && booleanValue2) {
						dvc = helper.createIntegerConstraint(opTypeMap.get(dataVeValue.getString("type2")),
								dataVeValue.getString("value1"), dataVeValue.getString("value2"));
					} else {
						dvc = helper.createDecimalConstraint(opTypeMap.get(dataVeValue.getString("type2")),
								dataVeValue.getString("value1"), dataVeValue.getString("value2"));
					}
				...
				if ("number_integer".equals(dataVeValue.getString("type"))
					...
					DataValidationConstraint dvc = helper.createNumericConstraint(
							opTypeMap.get(dataVeValue.getString("type")), opTypeMap.get(dataVeValue.getString("type2")),
							dataVeValue.getString("value1"), dataVeValue.getString("value2"));
					dstDataValidation = helper.createValidation(dvc, dstAddrList);
				}
				if ("date".equals(dataVeValue.getString("type"))) {
					// 日期
					DataValidationConstraint dvc = new XSSFDataValidationConstraint(
							opTypeMap.get(dataVeValue.getString("type")), opTypeMap.get(dataVeValue.getString("type2")),
							dataVeValue.getString("value1"), dataVeValue.getString("value2"));
					dstDataValidation = helper.createValidation(dvc, dstAddrList);
				}
